...
exports.build = () => {
  ...
  let _raw = `'Source ${name} traffic volume greater than ${dataVolume} in ${timeWindow}'`;
  const add = [
  ...
  if(__workerGroup) {
    ...
    _raw = `'Source ${name} in group ${__workerGroup} traffic volume greater than ${dataVolume} in ${timeWindow}'`;
  }
    ...
    pipeline: {
      conf: {
        functions: [
          ...
          {
            id: 'drop',
            filter: `bytes < ${dataVolumeBytes} || bytes === undefined`,
          },
          {
            id: 'eval',
            conf: {
